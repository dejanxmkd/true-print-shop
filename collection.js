...
document.querySelector('.catalog-hero .eyebrow')?.remove();
function render(){let items=catalog.products.filter(matches);if(sort==='low')items.sort((a,b)=>a[2]-b[2]);if(sort==='high')items.sort((a,b)=>b[2]-a[2]);if(sort==='az')items.sort((a,b)=>a[1].localeCompare(b[1]));const activeName=brand||categoryName(category);byId('collection-title').textContent=activeName;byId('result-count').textContent=`${items.length} products`;byId('catalog-grid').innerHTML=items.map(productCard).join('')||'<p>No products in this selection yet.</p>';document.title=`${activeName} — True Print Shop`}
